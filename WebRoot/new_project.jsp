...
<div class="main-container ace-save-state" id="main-container">
	...
	<a href="#" id="btn-scroll-up" class="btn-scroll-up btn btn-sm btn-inverse">
		...
		<script type="text/javascript">
			...
			//预览模态框的内容div的id值----angular.element（）获取元素
		        ...
		        $scope.createModel=function(){
			     ...
			     $scope.viewData=function(){
			    	$.post("readFileToShow_dynamicView.action",{//scope():检索当前元素或其父元素的scope。
						...
						if(pos==0&&($("#gender").val()==='是')){//判断一下如果是初始加载，并且含有表头，就为文件预览的表头添加复选框来实现用户点击复选框完成参数（公式）的填充
							...
							$('#table_body').append(checkBoxsStr);//在被选元素的结尾（仍然在内部）插入指定内容。
						} 
